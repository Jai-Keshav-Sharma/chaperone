...
| Ledger | SHA-256 hash chain (`sha2`) + RFC 6962 Merkle checkpoints + `ed25519-dalek` signing + optional Rekor v2 / RFC 3161 TSA anchoring (`reqwest`); storage: SQLite (`rusqlite`) default, Postgres (`sqlx`) for fleet mode |
| Interceptors | Same binary: `warden hook` (Claude Code / Cursor PreToolUse), `warden gateway` (MCP streamable-HTTP proxy), `warden shim` (MCP stdio proxy); ~1ms startup, fail-closed |
...
| Packaging | Single static binary per platform (prebuilt releases + `cargo install` + brew); optional `npx` shim later for hook distribution |
| License | Apache-2.0 |
