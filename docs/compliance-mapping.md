...
repo's corrected citations. Honest-claims rule applies here as everywhere: Warden MAPS
to standards; it does not "certify" anything — conformance claims follow the same
discipline as docs/aarm-mapping.md.

## The one-line story

Warden is the enforcement machine for controls that OWASP, NIST, CSA, the EU AI Act,
...
(transport) are NOT Warden's scope — Warden owns the tool-execution layer between them.

## NIST AI Risk Management Framework

| Function | Warden's operationalization |
|---|---|
...
Warden positions as a reference implementation ahead of it, without claiming
alignment to an unpublished document.

## Cloud Security Alliance

- **AICM v1.1** (Jun 2026, 247 controls): Warden implements the 24 agent-specific
  controls of the Agentic Control Supplement.
- **AIGF v1** (Agent Identity Governance Framework): Warden consumes its five
...
- **AGMM** (maturity model): Warden targets Level 3–4 organizations — the "84% fail a
...
| Article | Requirement | Warden artifact |
|---|---|---|
...
| Art. 12 | Automatic tamper-evident logging, 6-month retention | **The ledger**: hash chain + signed checkpoints + anchoring; `warden ledger export --format eu-ai-act` |
...
one system — Warden's unified cross-agent ledger satisfies the traceability demand
...
and buyer security questionnaires — not EU fines. `warden ledger export --format soc2`
...
- WIMSE: Warden CONSUMES WIT/WPT — validates workload identity on intercepted calls;
  never replaces the identity layer.
- MCP 2026-07-28 + EMA: EMA makes the IdP authoritative for server access (ID-JAG
  exchange, no traffic inspection). Warden's line: "AuthN/coarse-grant lives in the
  IdP (EMA); per-call AuthZ + proof lives in Warden."
...
- MeitY's India AI Governance Guidelines (Feb 2026, seven sutras) and sectoral
  ...
  research (Jun 2026) documents it. Warden = the first-mover implementation story for
