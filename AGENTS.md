...
- Changelog: conventional commits are MANDATORY (feat/fix/perf/refactor/revert/docs/test/chore).
  CHANGELOG.md is GENERATED by git-cliff (`make changelog` / `git cliff --latest`) from
  commit history — never hand-edit it. Every user-facing change must have a real,
  changelog-quality commit message, because the changelog IS the commit history.
